...
class XtreamClient:
    ...
    def get_series_episodes(self, series_id: Any, series_name: str) -> List[Dict[str, Any]]:
        ...
        `name` mirrors Xtream's own native series naming convention:
        "<series name> S<season>E<episode> - <title>". Grouping (which
        Xtream category this belongs to) is intentionally NOT decided
        here — the caller (series_generator) annotates each episode with
        the series' actual Xtream category as group_title, so the output
        is a verbatim reflection of Xtream's own Series section rather
        than a custom reorganization.
        """
