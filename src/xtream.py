...
class XtreamClient:
    ...
    def get_series_episodes(self, series_id: Any, series_name: str) -> List[Dict[str, Any]]:
        ...
        series_name, stream_icon, epg_channel_id}.

        `name` is pre-formatted as "SxxExx - <episode title>" (no series
        name prefix) — the caller uses `series_name` as the M3U
        group-title for these entries, so IPTV players fold a series'
        episodes into their own per-series folder rather than mixing
        every series in a category into one flat list. This mirrors how
        Xtream's own client browses Series: category -> series -> episodes.
        """
    ...
    def _flatten_episode(
        ...
        display_name = f"S{season_num:02d}E{episode_num:02d}"
        if title:
