...
class XtreamClient:
    ...
    def build_live_url(self, stream: Dict[str, Any]) -> Optional[str]:
        ...
        If Xtream omits container_extension for this channel (common for
        Live — many panels simply don't populate it), ".ts" is used. This
        is not a guess: Xtream Codes' own Live TV protocol always serves
        over an MPEG transport stream at that path regardless of whether
        the field is populated, and every standard Xtream client relies
        on the same convention. VOD/Series have no equivalent universal
        default, so no fallback is applied there.
        """
        direct = self._direct_source(stream)
        if direct:
            return direct
        return self._build_url(
            "live", stream.get("stream_id"), stream.get("container_extension"), fallback_ext="ts"
        )
    ...
    def get_series_episodes(self, series_id: Any, series_name: str) -> List[Dict[str, Any]]:
        ...
        series_name, group_title, stream_icon, epg_channel_id}.

        Xtream structures series as Series -> Season -> Episodes, but M3U
        group-title only supports one flat grouping level. To avoid every
        season of a series being mixed into one list, `group_title` is
        set to "<series name> - Season <n>" so each season becomes its
        own folder in the player, and `name` is just "E<nn> - <title>"
        since the season is already implied by that folder.
        """
    ...
    def _flatten_episode(
        ...
        display_name = f"E{episode_num:02d}"
        if title:
        ...
        return {
            ...
            "group_title": f"{series_name} - Season {season_num}",
    ...
    def _build_url(
        self, section: str, item_id: Optional[Any], extension: Optional[str], fallback_ext: Optional[str] = None
    ) -> Optional[str]:
        """
        Build a `{host}/{section}/{user}/{pass}/{id}.{ext}` URL using
        *exactly* the container_extension Xtream reports for this item.
        No format is ever guessed except `fallback_ext`, which callers
        only pass for Live (Xtream's own protocol convention — see
        build_live_url). VOD and Series pass no fallback, so a missing
        extension there causes the item to be skipped rather than
        emitted with a made-up, possibly wrong extension.

        Returns None if item_id is missing, or if no usable extension is
        available even after the fallback.
        """
        if item_id is None:
            return None
